...
- **Three arenas ship; the host picks one in the lobby.** The chosen `mapId` syncs on the room schema and the client resolves it from the shared registry. *This revises the MVP non-goal "more than one arena map"* — justified because the parser, not the map count, is the actual work. Shipped in S13 as Dojo, Twin Halls and Bamboo Grove; all three are **15x8**, so the client sizes its canvas once and a map choice never resizes the game. Map choice is **lobby-only** for the same reason — a room never returns to the lobby phase, so a rematch keeps its arena.
