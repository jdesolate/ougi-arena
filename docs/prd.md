...
- FR-22: Every ninja carries one melee weapon, picked at character select independently of character (character decides the Ougi, weapon decides the attack). At least 3 weapons ship: **kunai** (1 cell in front, fastest, lowest damage), **paper fan** (3 cells — front, front-left, front-right, medium speed and damage), **longsword** (2 cells — front and one beyond, slowest, highest damage). **No weapon moves what it hits** — a swing is damage and nothing else, so reach, coverage and speed are the only axes the three are traded on. *(Revised 2026-07-28 at the user's request: S23 had given the fan a one-cell shove, removed along with Shockwave's fling; the longsword's arc animation was also narrowed to a two-cell thrust, since it never hit the cells beside it.)*
...
- FR-28: A dash resolves to the nearest cell centre along its axis, clamped down to the nearest cell the player's TP can actually reach. A ninja knocked off-grid (an idle bump, a dash hard-stopped mid-cell by a pillar) is realigned by its next dash — snapping the destination, not the position, means no ninja can be wedged between cells.
