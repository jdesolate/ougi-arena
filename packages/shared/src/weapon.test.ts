...
describe("no weapon moves what it hits", () => {
  it("leaves every victim of every weapon exactly where it stood", () => {
    for (const weaponId of [KUNAI, FAN, LONGSWORD]) {
      ...
      // A hit is damage and nothing else: no shove, so nothing to travel and nothing to realign.
  ...
  it("holds all three of a fan's victims in place, including the two beside its front cell", () => {
    const state = arena([FAN, KUNAI, KUNAI, KUNAI]);
    const [a, b, c, d] = state.ninjas as [NinjaState, NinjaState, NinjaState, NinjaState];
    place(a, 240, 400);
    place(b, 240 + CELL, 400);
    ...
    for (const victim of [b, c, d]) {
      expect(victim.x).toBe(240 + CELL);
      expect(victim.hp).toBe(MAX_HP - FAN_DAMAGE);
      expect(victim.dashBudget).toBe(0);
    }
    expect(c.y).toBe(400 - CELL);
    expect(d.y).toBe(400 + CELL);
  });

  it("cannot move an off-grid victim onto a centre, since a swing never pushes at all", () => {
    const state = arena([FAN, KUNAI]);
    const [a, b] = state.ninjas as [NinjaState, NinjaState];
    place(a, 240, 400);
    // Parked 31 units into the cell in front, as a pillar hard-stop would leave it. It stays there.
    place(b, 240 + CELL + 31, 400);

    swingAndSettle(state, "a", 1, 0);

    expect(b.x).toBe(240 + CELL + 31);
  });
});
